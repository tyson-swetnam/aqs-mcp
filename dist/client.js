 ...
 */
export function validateDateRange(bdate, edate) {
    const beginYear = bdate.substring(0, 4);
    const endYear = edate.substring(0, 4);
    if (beginYear !== endYear) {
        throw new Error(`Begin date (${bdate}) and end date (${edate}) must be in the same calendar year. ` +
 ...
 */
export function validateDateFormat(date, fieldName) {
    if (!/^\d{8}$/.test(date)) {
        throw new Error(`${fieldName} must be in YYYYMMDD format. Got: ${date}`);
    }
 ...
 */
export async function aqs(endpoint, params) {
 ...
 */
export function resolveCredentials(providedEmail, providedKey) {
