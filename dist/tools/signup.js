...
export const signupTool = {
    name: 'aqs_signup',
    description: 'Register for an EPA Air Quality System (AQS) API key. ' +
    ...
    handler: async (args) => {
        ...
        try {
            const response = await aqs('signup', { email });
            ...
            if (status === 'Success' || status.toLowerCase().includes('success')) {
                return {
                    content: [
                        {
                            type: 'text',
                            text: `API key registration successful!\n\n` +
 ...
 */
export const isAvailableTool = {
    name: 'aqs_is_available',
    description: 'Check if the EPA Air Quality System (AQS) API is operational. ' +
    ...
    inputSchema: {
        ...
        properties: {
            email: {
                type: 'string',
                description: 'Email address for API authentication (optional, falls back to AQS_EMAIL env var)',
            },
            key: {
                type: 'string',
                description: 'API key for authentication (optional, falls back to AQS_API_KEY env var)',
            },
        },
    },
    handler: async (args) => {
        try {
            ...
            const response = await aqs('metaData/isAvailable', {
                email,
                key,
            });
            const header = response.Header?.[0];
            ...
            if (status === 'Success' || status.toLowerCase().includes('success')) {
                return {
                    content: [
                        {
                            type: 'text',
                            text: `EPA AQS API Status: AVAILABLE\n\n` +
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `EPA AQS API Status: ${status}\n\n` +
            ...
            if (errorMessage.includes('Email is required') ||
                ...
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Credentials required: ${errorMessage}\n\n` +
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `EPA AQS API Status: UNAVAILABLE or ERROR\n\n` +
                            `Error: ${errorMessage}`,
                    },
                ],
            };
 ...
 */
export const signupTools = [signupTool, isAvailableTool];
