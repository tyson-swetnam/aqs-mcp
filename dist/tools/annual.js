 ...
 */
import { aqs, resolveCredentials, validateDateFormat, validateDateRange, } from '../client.js';
/**
...
const credentialProperties = {
    email: {
        type: 'string',
        description: 'Email address registered with the AQS API. If not provided, uses AQS_EMAIL environment variable.',
    },
    key: {
        type: 'string',
        description: 'API key for AQS access. If not provided, uses AQS_API_KEY environment variable.',
    },
};
/**
...
const dateRangeProperties = {
    param: {
        type: 'string',
        description: 'Parameter code for the pollutant (e.g., "44201" for Ozone, "88101" for PM2.5, "42401" for SO2, "42101" for CO, "42602" for NO2, "81102" for PM10). Up to 5 comma-separated codes allowed.',
    },
    bdate: {
        type: 'string',
 ...
 */
function formatResponse(data, rowCount) {
 ...
 */
async function handleAnnualRequest(endpoint, params, locationParams) {
    try {
        // Resolve credentials
        ...
        // Make API request
        const response = await aqs(endpoint, requestParams);
