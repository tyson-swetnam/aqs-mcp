 ...
 */
const listStatesTool = {
    name: 'aqs_list_states',
    description: 'Get a list of all US states with their 2-digit FIPS codes. ' +
    ...
    handler: async (args) => {
        ...
        const response = await aqs('list/states', { email, key });
 ...
 */
const listCountiesTool = {
    name: 'aqs_list_counties',
    description: 'Get a list of counties within a state with their 3-digit FIPS codes. ' +
    ...
    handler: async (args) => {
        const { email, key } = resolveCredentials(args.email, args.key);
        const response = await aqs('list/countiesByState', {
            email,
            key,
            state: args.state,
        });
...
const listSitesTool = {
    name: 'aqs_list_sites',
    description: 'Get a list of air quality monitoring sites within a county with their 4-digit site codes. ' +
    ...
    handler: async (args) => {
        const { email, key } = resolveCredentials(args.email, args.key);
        const response = await aqs('list/sitesByCounty', {
            email,
            key,
            state: args.state,
            county: args.county,
...
const listCbsasTool = {
    name: 'aqs_list_cbsas',
    description: 'Get a list of Core Based Statistical Areas (CBSAs) with their codes. ' +
    ...
    handler: async (args) => {
        const { email, key } = resolveCredentials(args.email, args.key);
        const response = await aqs('list/cbsas', { email, key });
        return formatResponse(response.Data);
    },
};
/**
 ...
 */
const listParameterClassesTool = {
    name: 'aqs_list_parameter_classes',
    description: 'Get a list of parameter classification groups (e.g., "CRITERIA", "AIR TOXICS", "METEOROLOGICAL"). ' +
    ...
    handler: async (args) => {
        const { email, key } = resolveCredentials(args.email, args.key);
        const response = await aqs('list/classes', { email, key });
        return formatResponse(response.Data);
    },
};
/**
 ...
 */
const listParametersTool = {
    name: 'aqs_list_parameters',
    description: 'Get a list of parameters (pollutants/measurements) within a parameter class. ' +
    ...
    inputSchema: {
        ...
        properties: {
            ...
            pc: {
                type: 'string',
                description: 'Parameter class name (e.g., "CRITERIA", "AIR TOXICS", "METEOROLOGICAL"). ' +
    ...
    handler: async (args) => {
        const { email, key } = resolveCredentials(args.email, args.key);
        const response = await aqs('list/parametersByClass', {
            email,
            key,
            pc: args.pc,
        });
